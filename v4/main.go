// v4: use functions instead of methods
// https://play.golang.org/p/sTMEyQcLzk1
...
func BuyEggs(s Shopper, howMany int, err error) (Shopper, error) {
	if err != nil {
		return s, err
	}
...
func main() {
	...
	shopper, err := Drive(shopper, FuelNeededToGetToStore, nil)

	shopper, err = BuyEggs(shopper, EggsRequired, err)

	shopper, err = Drive(shopper, FuelNeededToGetHome, err)
